...
void spPrint (eMatrix, PrintReordered, Data, Header, fp)
char *eMatrix;
int PrintReordered, Data, Header;
FILE *fp;
{
  ...
  if (Header)
  {
    ...
    fprintf (fp, "Size of matrix = %1u x %1u.\n", Size, Size);
    if (Matrix->Reordered AND PrintReordered)
 ...
 */
    ...
    /* Calculate index of last column to printed in this group. */
  {
	...
	for (I = StartCol; I <= StopCol; I++)
	{
	  if (PrintReordered)
	    ...
	    Col = PrintOrdToIntColMap[I];
	  fprintf (fp, " %9d", Matrix->IntToExtColMap[Col]);
	}
	fprintf (fp, "\n\n");
      }
      else
	...
	if (PrintReordered AND NOT Data)
	  ...
	  fprintf (fp, "%4d", Matrix->IntToExtRowMap[Row]);
	if (NOT Data)
	  fprintf (fp, " ");
      }
	...
	{
	  if (Data)
	    fprintf (fp, " %9.3lg", (double) pElement->Real);
	  else
	    fprintf (fp, "x");
  ...
  {
    fprintf (fp, "\nLargest element in matrix = %-1.4lg.\n", LargestElement);
    fprintf (fp, "Smallest element in matrix = %-1.4lg.\n", SmallestElement);
    ...
    {
      fprintf (fp, "\nLargest diagonal element = %-1.4lg.\n", LargestDiag);
      fprintf (fp, "Smallest diagonal element = %-1.4lg.\n", SmallestDiag);
    }
    else
    {
      fprintf (fp, "\nLargest pivot element = %-1.4lg.\n", LargestDiag);
      fprintf (fp, "Smallest pivot element = %-1.4lg.\n", SmallestDiag);
    }
...
int spFileMatrix (eMatrix, File, Label, Reordered, Data, Header)
...
int spFileVector (eMatrix, File, RHS IMAG_RHS)
char *eMatrix, *File;
...
int spFileStats (eMatrix, File, Label)
